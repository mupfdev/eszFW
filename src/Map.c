...
int InitMap(const char *pacFileName, Map **pstMap)
{
    *pstMap = malloc(sizeof(struct Map_t));
    ...
    for (uint16_t u16Index = 0; u16Index < MAP_LAYERS; u16Index++)
    {
        (*pstMap)->pstLayer[u16Index] = NULL;
    }
    ...
    SDL_Renderer **pstRenderer)
{
    ...
    // The layer has already been rendered and can be drawn.
    if ((*pstMap)->pstLayer[u16Index])
    {
    ...
    // Otherwise render the layer once.
    (*pstMap)->pstLayer[u16Index] = SDL_CreateTexture(
        (*pstRenderer),
    ...
    if (NULL == (*pstMap)->pstLayer[u16Index])
    {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "%s\n", SDL_GetError());
        return -1;
    }
    ...
    if (0 != SDL_SetRenderTarget((*pstRenderer), (*pstMap)->pstLayer[u16Index]))
    {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "%s\n", SDL_GetError());
    ...
    if (0 != SDL_SetTextureBlendMode((*pstMap)->pstLayer[u16Index], SDL_BLENDMODE_BLEND))
    {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "%s\n", SDL_GetError());
        return -1;
    }
